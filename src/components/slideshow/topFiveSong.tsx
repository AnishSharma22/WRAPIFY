...
import { useNavigate } from 'react-router-dom';
import { useRecoilState } from 'recoil';
import { longTrackState, userState } from '@/recoil/userState';
  ...
  const TopFiveSong = () => {
    ...
    const [longTrack] = useRecoilState(longTrackState);
      ...
      <div className='super-div py-20'>
        ...
        <div className='pager'>
          ...
          <a> <img className='icons press mr-10' src={sharePNG} style={{ height: '25px', alignSelf: 'end', position: 'absolute', top: '2rem', right: '0' }} alt="Share" /> </a>
            <p>And these are your top 5 masterpieces of all time!</p>
          </div>
          ...
          <div className='imagePlace' style={{ backgroundImage: `url(${longTrack[0].album.images[0].url})`, height:'4.2rem'}}>
          <img
                ref={imageRef}
                src={longTrack[2].album.images[0].url}
                alt=""
                crossOrigin="anonymous"
                style={{visibility:"hidden"}}
              />
             </div>
              <li>&nbsp;{longTrack[0].name}</li>
              <p className='artist'>&nbsp; {longTrack[0].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < longTrack[0].artists.length - 1 && ', '}
                                          </span>
                                        ))}
                                        </p>
              <div className='imagePlace' style={{ backgroundImage: `url(${longTrack[1].album.images[0].url})`, top: '4.2rem'}}> </div>
              <li> &nbsp;{longTrack[1].name}</li>
              <p className='artist'>&nbsp; {longTrack[1].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < longTrack[1].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${longTrack[2].album.images[0].url})`, top: '7.7rem'}}> </div>
              <li>&nbsp;{longTrack[2].name}</li>
              <p className='artist'>&nbsp; {longTrack[2].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < longTrack[2].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${longTrack[3].album.images[0].url})`, top: '11.2rem'}}> </div>
              <li>&nbsp;{longTrack[3].name}</li>
              <p className='artist'>&nbsp; {longTrack[3].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < longTrack[3].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${longTrack[4].album.images[0].url})`, top: '14.7rem'}}> </div>
              <li>&nbsp;{longTrack[4].name}</li>
              <p className='artist'>&nbsp; {longTrack[4].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < longTrack[4].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
          ...
          <div className='wrapper' style={{ marginTop: '1rem' }}>
            ...
            <a onClick={handleButtonClick} style={hasButtonBeenClicked ? filterStyle : {}}  ><img src={heartSVG} alt="Like" className='icons heart press icons w-6' style={{ minWidth: '1.5rem'}} /> </a>
              <div className='fader text-white' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '20rem' }}>
                <p style={{ fontSize: '1.3em', textAlign: 'center', width: '20rem' }}>Top 5 Songs of All Time!</p>
            ...
            <div className='grid fader justify-center items-center'>
              ...
              <input type="range" min="1" max="100" value="91" className='slider' style={{   background: 'linear-gradient(to right, #ffffff 2%, #B287D4 91%)', right: '0.9rem'}}/>
          ...
          <div className='flex justify-center items-start my-12 z-10'>
            ...
            <a className='press' > <img className='icons' src={backPNG} style={{ height: '25px', marginRight: '3rem' }} alt="" /></a>
              ...
              <button className='press' onClick={()=>navigate('/thirteen')}>
