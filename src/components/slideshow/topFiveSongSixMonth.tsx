...
import { useNavigate } from 'react-router-dom';
import { useRecoilState } from 'recoil';
import { mediumTrackState, userState } from '@/recoil/userState';



  const TopFiveSongSixMonth = () => {

    const imageRef = useRef(null);
    const [user] = useRecoilState(userState);
    const[mediumTrack] = useRecoilState(mediumTrackState); 
      ...
      <div className='super-div py-20'>
        ...
        <div className='pager'>
          ...
          <a> <img className='icons press mr-10' src={sharePNG} style={{ height: '25px', alignSelf: 'end', position: 'absolute', top: '2rem', right: '0' }} alt="Share" /> </a>
            <p>These are your top 5 songs of last 6 months</p>
          </div>
          ...
          <div className='imagePlace' style={{ backgroundImage: `url(${mediumTrack[0].album.images[0].url})`, height:'4.2rem'}}>
          <img
                ref={imageRef}
                src={mediumTrack[2].album.images[0].url}
                alt=""
              ...
              <li>&nbsp;{mediumTrack[0].name}</li>
              <p className='artist'>&nbsp; {mediumTrack[0].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < mediumTrack[0].artists.length - 1 && ', '}
                                          </span>
                                        ))}
                                        </p>
              <div className='imagePlace' style={{ backgroundImage: `url(${mediumTrack[1].album.images[0].url})`, top: '4.2rem'}}> </div>
              <li> &nbsp;{mediumTrack[1].name}</li>
              <p className='artist'>&nbsp; {mediumTrack[1].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < mediumTrack[1].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${mediumTrack[2].album.images[0].url})`, top: '7.7rem'}} ref={imageRef}> 
              </div>
              <li>&nbsp;{mediumTrack[2].name}</li>
              <p className='artist'>&nbsp; {mediumTrack[2].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < mediumTrack[2].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${mediumTrack[3].album.images[0].url})`, top: '11.2rem'}}> </div>
              <li>&nbsp;{mediumTrack[3].name}</li>
              <p className='artist'>&nbsp; {mediumTrack[3].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < mediumTrack[3].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
              <div className='imagePlace' style={{ backgroundImage: `url(${mediumTrack[4].album.images[0].url})`, top: '14.7rem'}}> </div>
              <li>&nbsp;{mediumTrack[4].name}</li>
              <p className='artist'>&nbsp; {mediumTrack[4].artists.map((artist, index) => (
                                          <span key={index}>
                                            {artist.name}
                                            {index < mediumTrack[4].artists.length - 1 && ', '}
                                          </span>
                                        ))}</p>
          ...
          <div className='wrapper' style={{ marginTop: '1rem' }}>
            ...
            <a onClick={handleButtonClick} style={hasButtonBeenClicked ? filterStyle : {}}  ><img src={heartSVG} alt="Like" className='icons heart press  icons w-6' style={{ minWidth: '1.5rem'}} /> </a>
              <div className='fader text-white' style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', maxWidth: '20rem' }}>
                <p style={{ fontSize: '1.3em', textAlign: 'center', width: '20rem' }}>Top 5 Songs of Six Months!</p>
            ...
            <div className='grid fader justify-center items-center'>
              ...
              <input type="range" min="1" max="100" value="42" className='slider' style={{   background: 'linear-gradient(to right, #ffffff 2%, #B287D4 42%)', right: '0.9rem'}}/>
          ...
          <div className='flex justify-center items-start my-12 z-10'>
            ...
            <a className='press' > <img className='icons' src={backPNG} style={{ height: '25px', marginRight: '3rem' }} alt="" /></a>
              ...
              <button className='press' onClick={()=>navigate('/seventh')}>
  ...
  export default TopFiveSongSixMonth;
